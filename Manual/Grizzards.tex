...
to the \emph{left} controller port\ifdefined\ATARIAGESAVE, and (optionally) your
AtariVox device to the \emph{right} controller port.  \else\ifdefined\NOSAVE.\else{}, and the
memory device to the \emph{right} controller port.
...
abandon your progress and return to the Title Screen. You will lose some
of your progress since the last time you visited a Grizzard Depot.

\subsection{Difficulty Switches}

\ifdefined\NOSAVE\else

You cannot erase a game in  progress unless both Difficulty Switches are
in the  ``A'' (Advanced or Expert)  position. To protect your  game from
being erased,  set either one of  the Difficulty Switches to  the ``B''
...
\textbf{B} button as the \textbf{Fire} button. Use the \textbf{C} button
as an alternative way to press the \textbf{Game Select} switch to access
your stats.

Your gamepad  \emph{must} be  plugged in \emph{before}  you turn  on the
power  if you  want to  use it.  Otherwise, the  \textbf{C} button  will
be ignored.
...
slot\ifdefined\ATARIAGESAVE\else\footnote{Technical   Note:   The   Slot
  ...
  occupies 4 blocks on your memory device.}\fi for you
r game. There are
...
If   someone    has   already    \emph{won}   the   game    once,   then
``\texttt{RESUME}'' and their name will appear in bright yellow. See New
Game Plus.
...
Press the  joystick forward (up)  to select  the next letter;  pull back
(down) to select the previous letter.  Press the joystick left and right
to  move to  the next  or  previous position.  You  can have  up to  six
characters for  your name.  After six characters,  the cursor  will turn
pink. Press the \textbf{Fire} button to begin your adventure.
...
WRITEME for more information about each of them.

WRITEME --- want some screenshots here.

Press the joystick  left and right to choose a  Grizzard companion, then
press the \textbf{Fire} button to continue.

You'll have one  last chance to make sure that  you've entered your name
correctly  and  chosen the  Grizzard  companion  you prefer.  Press  the
joystick up to select \texttt{CHANGE}, or down to select \texttt{BEGIN},
and press the \textbf{Fire} button.
...
In this demo, you can only play with one Grizzard at a time --- starting
with Aquax.  The demo using  a memory device  allows you to  catch other
Grizzards.  In the  full  game, you  can choose  one  of three  starting
Grizzard companions.

\fi

You'll begin in the ruins of  Treble Village after the monsters invaded.
You know that to the east are  the extremely dangerous Fire Bog, so your
...
If you have a  potion, you can give it to your  Grizzard by pressing the
\textbf{Fire} button.  \ifdefined\DEMO However, there are  no potions in
this demo. \fi
...
other people,  signposts, or doors.  To interact with them,  simply walk
into them.
...
beginning with Aquax. With a memory  device or the retail version of the
game cartridge, you may encounter  other Grizzards that you can convince
to join your party.
...
party,   with   up   to   30   different   Grizzards   saved   in   your
\ifdefined\ATARIAGESAVE game  cartridge. \else  memory device.  \fi Each
Grizzard may have its own moves and skill scores.
...
When  you  walk  through a  door,  you'll  emerge  in  front of  a  door
somewhere else.
...
\begin{center}
  \includegraphics[width=\columnwidth]{../Manual/Talking\TV.png}
  *** Missing art: talking
\end{center}
...
\begin{center}
  \includegraphics[width=\columnwidth]{../Manual/Inquiry\TV.png}
  *** Missing art: Inquiry
\end{center}
...
If your Grizzard  knows how to perform  a move, it will  appear in blue.
...
be injured  (lose hit points) or  have a Status Effect  that changes its
statistics. Status Effects  are temporary and last only  the duration of
one battle.
...
Grizzards to catch, numbered 1-30. Can you catch them all?
...
They  may go  up a  bit after  each monster  that you  defeat. The  more
skilled your Grizzard becomes, the less often its levels will go up.
...
These  status  effects last  only  the  duration  of one  battle;  after
a  battle,  any survivors  will  no  longer  have these  Status  Effects
upon them.

Also, these  status effects cannot be  added on top of  the same effect.
That means,  for example: Once your  attack has been raised,  you cannot
...
Switches to  the ``A'' (Advanced or  Export) position, pull down  on the
joystick, and hold the \textbf{Fire} button.
...
available with a memory device or the retail cartridge.
...
while on the  map screen. \ifdefined\DEMO However, there  are no potions
in this demo. \fi
...
\lettrine[image=true, lines=5, findent=3pt, nindent=3pt]{../Manual/Creepy-Spider.png}{Creepy Spiders}
are much bigger than  the sort where you come from.  Watch out for those
giant mandibles, they hurt!
...
Grizzard's stats and effectively pause the game as well.

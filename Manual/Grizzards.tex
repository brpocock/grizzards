...
controller (or SEGA  \ifdefined\TVNTSC{Genesis/}\fi{}MegaDrive gamepad) to
the  \emph{left}  controller   port,  and  the  memory   device  to  the
\emph{right} controller port.  Make certain that the Power  switch is in
the Off position when connecting your memory device.
...
\textbf{B} button  as the Fire button.  Use the \textbf{C} button  as an

...
\fi\ifdefined\DEMO Demo \fi\ifdefined\ARIA\else Public Release\fi Player's Guide
...
Video  Arcade or  Atari  7800 ProSystem)  \ifdefined\ARIA with  optional
AtariVox device. \else \ifdefined\NOSAVE without \else
with \fi AtariVox (or MemCard, or SaveKey) device. \fi
...
\ifdefined\ARIA\else
If  you  enjoy playing  \textif{Grizzards},  you  may be  interested  in
purchasing a retail copy with built-in save-game memory from AtariAge.
\fi
...
to the \emph{left} controller port. \ifdefined\ARIA, and (optionally) your
AtariVox device to the \emph{right} controller port. Make certain
that  the \textbf{Power}  switch  is in  the \textbf{Off}  position  when connecting  your
AtariVox device. \else\ifdefined\NOSAVE\else{}, and the
...
require  a memory  device  because the  special \textit{AtariAge}  cartridge
contains its own save-game memory within it.

To purchase \textit{Grizzards} or many other Atari games, visit \href{https://atariage.com/}{https://atariage.com/}
\fi\fi

Finally, insert  the \textit{Grizzards}  game cartridge (with  the label
facing up) into the cartridge slot, and turn the \textbf{Power} \textbf{On}.
...
right to choose  a memory slot\footnote{Technical Note:  The Slot number
  chosen here  is relative to  the \ifdefined\ARIA four \else  three \fi
  save  game   slots  used  by  the   \textit{Grizzards}  game  program.
  \ifdefined\ARIA\else Each save game slot actually occupies 4 blocks on
  your memory device. \fi} for your game. There are \ifdefined\ARIA four
\else three \fi memory slots possible. Press \textbf{Game Select} or the
joystick again to  rotate through them. If someone has  already begun to
play  \textit{Grizzards}  in  a  certain slot,  your  screen  will  show
...
(down) to select  the previous letter. Press the joystick  right to move
to the  next letter; press  left to move  back. When you've  entered six
characters  for  your name,  press  the  \textbf{Fire} button  to  begin
your adventure.

\fi

You'll begin in the ruins of  Treble Village after the monsters invaded.
You know that to the east are the extremely dangerous \textbf{Fire} Bog,
so  your best  bet is  to head  west  (left) and  see if  there are  any
other survivors.
...
party, with up to 30 total  Grizzards saved in your \ifdefined\ARIA save
file on the game cartridge. \else memory device. \fi
...
\ifdefined\NOSAVE\else
Your progress  will \emph{immediately} be saved  to your \ifdefined\ARIA
game cartridge. \else memory device. \fi
\fi
...
bar will change to a red color.
...
Novice) position, then you can't erase a game slot. \ifdefined\ARIA\else
\emph{Tip: When  you connect your  memory device, check the  position of
  those switches.} \fi
...
\ifdefined\ARIA\else\ifdefined\NOSAVE\else
...
\ifdefined\NOSAVE\else\ifdefined\ARIA\else
...
Naturally, there are  no voices when playing  \ifdefined\ARIA without an
...
\section{Save to Cartridge Technology}

This  game uses  the advanced  capabilities of  AtariAge's \textit{AtariAge}
cartridge system to save your progress to an EEPROM chip inside the game
cartridge itself.

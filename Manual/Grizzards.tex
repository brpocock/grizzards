...
\item A joystick controller (or SEGA Genesis gamepad)
...
controller (or SEGA Genesis gamepad) to the \emph{left} controller port,
and the memory device to  the \emph{right} controller port. Make certain
...
switch to  review your Grizzard's  statistics from the Combat  screen or
a Grizzard Depot.

On the Map screen, the \textbf{Game Select} switch has no effect.
...
A SEGA Genesis  gamepad or other compatible controller may  also be used
with \textit{Grizzards}. Use  the \textbf{B} button as  the Fire button.
Use  the  \textbf{C}   button  as  an  alternative  way   to  press  the
\textbf{Game Select} switch.
...
Press the  \textbf{Game Select}  switch or  Fire button  to move  to the
Select Slot screen.
...
\textbf{Game Reset} switch or Fire button to start.
...
To review your Grizzard's statistics, press the \textbf{Game Select} switch.
...
You  can also  press the  \textbf{Game Select}  switch during  combat to
review your  Grizzard's statistics.  Each Grizzard  companion has  a few
statistics:
...
From the  Title Screen, press  \textbf{Game Select} or the  Fire button.
...
\includegraphics[width=\columnwidth]{../Manual/Dirtex\TV.png}
...
\includegraphics[width=\columnwidth]{../Manual/Airex\TV.png}
...
\includepdf[height=9in,width=6in,fitpaper=true]{../Manual/SyrexMap.pdf}

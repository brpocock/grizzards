...
joystick up  to select CHANGE,  or down to  select BEGIN, and  press the
\textbf{Fire} button.
...
\ifdefined\ATARIAGESAVE save  file on  the game cartridge.  \else memory
device. \fi Each Grizzard may have its own moves and skill scores.
...
visited by  resuming your  game's saved progress.  
\fi
...
(toward the TV). \ifdefined\DEMO
Your game record will be deleted \emph{immediately}.
...
After deleting  your saved game's  progress, return the  Left Difficulty
Switch to the ``B'' position to begin playing again.
